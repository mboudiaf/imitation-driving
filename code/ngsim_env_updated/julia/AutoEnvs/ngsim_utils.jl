...
function load_ngsim_trajdatas(filepaths; minlength::Int=100)
    ...
    println("indexes filepaths")
    println(indexes_filepaths)
    indexes =Dict[]
    for (i, index_filepath) in enumerate(indexes_filepaths)
	    ...
	    #index = JLD.convert(Base.Dict{Core.Any, Core.Any}, index)
	    ids_filepath = replace(index_filepath, ".jld", "-ids.h5")
	    println("reading index from h5...")
	    h5open(ids_filepath, "r") do file
	    	ids = read(file, "ids")
		ts  = read(file, "ts")
		te  = read(file, "te")
	    end
	    vals_vals = [ts, te]
	    vals_keys = ["ts", "te"]
	    vals = Dict(zip(vals_keys, vals_vals))
	    index = Dict(zip(ids, vals))
	    println("index read from h5!")
	    println("index type:")
	    println(typeof(index))
     	end
        # load index
        push!(indexes,index)
	println("index pushed to indexes")
    end
    print("All indexes loaded: corresponding to ",length(indexes))
    # load trajdatas
    ...
    println("Roadways of len ",length(roadways))
...
function sample_trajdata_vehicle(
	...
	println(length(trajinfos))
	#println(length(keys(trajinfos[traj_idx])))
