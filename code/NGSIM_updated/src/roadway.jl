...
function write_roadways_to_dxf()
    ...
    roadway_input_lankershim = RoadwayInputParams(Pkg.dir("NGSIM", "data", "boundariesLankershim.txt"),
                                                  Pkg.dir("NGSIM", "data", "centerlinesLankershim.txt"))

    # ngsimroadway_80 = read_roadway(roadway_input_80)
    # ngsimroadway_101 = read_roadway(roadway_input_101)
    ngsimroadway_lankershim = read_roadway(roadway_input_lankershim)

    # open(io->write_dxf(io, ngsimroadway_80), Pkg.dir("NGSIM", "data", "ngsim_80.dxf"), "w")
    # open(io->write_dxf(io, ngsimroadway_101), Pkg.dir("NGSIM", "data", "ngsim_101.dxf"), "w")
    open(io->write_dxf(io, ngsimroadway_lankershim), Pkg.dir("NGSIM", "data", "ngsim_lankershim.dxf"), "w")
end
function write_roadways_from_dxf()
    ...
    roadway_lankershim = open(io->read_dxf(io, Roadway, dist_threshold_lane_connect=2.0), Pkg.dir("NGSIM", "data", "ngsim_lankershim.dxf"), "r")

    # also converts to meters
    # convert_curves_feet_to_meters!(roadway_80)
    # convert_curves_feet_to_meters!(roadway_101)
    convert_curves_feet_to_meters!(roadway_lankershim)
    ...
    open(io->write(io, MIME"text/plain"(), roadway_lankershim), Pkg.dir("NGSIM", "data", "ngsim_lankershim.txt"), "w")
end
...
const ROADWAY_LANKERSHIM = open(io->read(io, MIME"text/plain"(), Roadway), Pkg.dir("NGSIM", "data", "ngsim_lankershim.txt"), "r")
