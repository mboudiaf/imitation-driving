...
mutable struct FilterTrajectoryResult
    ...
    function FilterTrajectoryResult(trajdata::NGSIMTrajdata, carid::Int)
        ...
        N = convert(Int, trajdata.df[dfstart, :n_frames_in_dataset])
...
function Base.convert(::Type{Trajdata}, tdraw::NGSIMTrajdata, roadway::Roadway)
    ...
    for frame in keys(tdraw.frame2cars)
        frame_lo = state_ind+1

        for car_id in carsinframe(tdraw, frame)
            dfind = convert(Int, car_df_index(tdraw, car_id, frame))
            if dfind==0
                println(frame)
                println(car_id)
                println(tdraw.frame2cars[frame])
                lo = tdraw.car2start[car_id]
                println(lo)
                framestart = df[lo, :frame]
                println(framestart)
                println(frame - framestart + lo -1)
                n_frames = df[lo, :n_frames_in_dataset]
                println(n_frames)
                println(length(df[df[:id].==car_id,:frame]))
                println(df[df[:id].==car_id,:frame])
            ...
            states[state_ind += 1] = RecordState(VehicleState(posG, roadway, speed), car_id)
        end
...
get_corresponding_roadway(filename::String) = contains(filename, "i101") ? ROADWAY_101 : (contains(filename, "80") ? ROADWAY_80 : ROADWAY_LANKERSHIM)
...
function convert_raw_ngsim_to_trajdatas()
    for filename in NGSIM_TRAJDATA_PATHS
        ...
        outpath = Pkg.dir("NGSIM", "data", "trajdata_lankershim_trajectories.txt")
...
const TRAJDATA_PATHS = [
                        Pkg.dir("NGSIM", "data", "trajdata_lankershim_trajectories.txt"),
