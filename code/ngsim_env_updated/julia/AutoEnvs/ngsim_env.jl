...
function _step!(env::NGSIMEnv, action::Array{Float64})
    ...
    pos=orig_veh.state.posG - env.ego_veh.state.posG
    step_infos["rmse_pos"] = sqrt(norm(VecE2(pos.x,pos.y))^2)
    vel=orig_veh.state.v - env.ego_veh.state.v
    step_infos["rmse_vel"] = sqrt(abs(vel)^2)
    lane_off = orig_veh.state.posF.t - env.ego_veh.state.posF.t
    step_infos["rmse_t"] = sqrt(abs(lane_off)^2)
