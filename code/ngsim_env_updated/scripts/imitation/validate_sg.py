...
def simulate(env, policy, max_steps, baseline, use_baseline=False, render=False, env_kwargs=dict()):
...
def collect_trajectories(
    ...
    with tf.Session() as sess:
        ...
        if use_baseline:
            baseline._regressor.fit(observations, returns.reshape((-1,2)))
...
def collect(
    ...
    if not use_baseline:
        ...
        output_filepath = os.path.join(validation_dir, '{}_trajectories_sg.npz'.format( args.ngsim_filename.split('.')[0]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--params_filename', type=str, default='itr_1000.npz')
    ...
    parser.add_argument('--use_baseline', type=str2bool, default=False)
...
#    ]
    ...
    for fn in filenames:
        ...
        collect(
            ...
            use_baseline=run_args.use_baseline
        )
