...
class PolicyNetwork():
    def __init__(self, args):
        ...
        self.inputs = tf.placeholder(
            tf.float32, [None, args.state_dim], name="inputs")
        self.targets = tf.placeholder(
            tf.float32, [None, args.action_dim], name="targets")
    ...
    def _create_mlp(self, args):
        ...
        if args.gru_input_dim > 0:
            ...
            b = tf.get_variable("mlp_policy/mean_network/output_flat/b",
                                [args.action_dim])
            self.a_mean = tf.nn.xw_plus_b(output, W, b,"a_mean_mlp")
    ...
    def _create_gru(self, args):
        ...
        self.W_x_ruc = tf.concat([W_xr, W_xu, W_xc],1)
        self.W_h_ruc = tf.concat([W_hr, W_hu, W_hc],1)
        self.b_ruc = tf.concat([b_r, b_u, b_c],0)
        ...
        self.h = tf.math.add((1 - self.u) * self.hprev, self.u * self.c,name="hidden_state")
        ...
        b = tf.get_variable("gru_policy/mean_network/output_flat/b",
                            [args.action_dim])
        self.a_mean = tf.nn.xw_plus_b(self.h, W, b,name="a_mean")
        #a_mean = tf.nn.xw_plus_b(self.h, W, b,name="a_mean")
        # Initialize logstd
        self.a_logstd = tf.Variable(np.zeros(
    ...
    def _create_optimizer(self, args):
        ...
        self.summary_policy = tf.summary.scalar(
