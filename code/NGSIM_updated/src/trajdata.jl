...
mutable struct FilterTrajectoryResult
    ...
    function FilterTrajectoryResult(trajdata::NGSIMTrajdata, carid::Int)
        ...
        k = min(N,5)
        θ_arr[1] = atan2(y_arr[k] - y_arr[1], x_arr[k] - x_arr[1])
    ...
    for frame in 1 : nframes(tdraw)
        ...
        for id in carsinframe(tdraw, frame)
            ...
            if dfind==0
                println("carid: ", id)
                println("frame: ", frame)
                println("car2star: ", tdraw.car2start[id])
                lo = tdraw.car2start[id]
                println("framestart: ", tdraw.df[lo, :frame])
                println("nframes: ", tdraw.df[lo, :n_frames_in_dataset] )
            end
            if id==115
                if frame==557
                    println("TEST1")
