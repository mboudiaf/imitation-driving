...
mutable struct NGSIMTrajdata
    ...
    function NGSIMTrajdata(input_path::String)
        ...
        df = contains(input_path,".txt") ? readtable(input_path, separator=' ', header = false) : readtable(input_path)
        col_names = [:id, :frame, :n_frames_in_dataset, :epoch, :local_x, :local_y, :global_x, :global_y, :length, :width, :class, :speed, :acc, :lane, :origin, :dest, :intersection, :section, :direction, :move, :carind_front, :carind_rear, :dist_headway, :time_headway]
        for (i,name) in enumerate(col_names)
            rename!(df, Symbol(@sprintf("x%d", i)), name)
        end
        #add a column "global_heading"
        df[:global_heading] = fill(NaN, nrow(df))
        sort!(df, [order(:frame), order(:id)])

        #car2start: car_id -> index of first row wher car_id appears
        #frame2cars: frame_id -> list of all the cars appearing in this frame
        car2start = Dict{Int, Int}()
        frame2cars = Dict{Int, Vector{Int}}()
        
        for (dfind, carid) in enumerate(df[:id])
            dfind = convert(Int, dfind)
            carid = convert(Int, carid)
...
Records.nframes(trajdata::NGSIMTrajdata) = length(keys(trajdata.frame2cars))
...
function car_df_index(trajdata::NGSIMTrajdata, carid::Int, frame::Int)
    ...
    elseif frame ≥ framestart
        retval = frame - framestart + lo -1 ##add a -1...
    ...
    # if carid in df[:id]
    #     dfid = df[df[:id].==carid,:]
    #     if frameid in dfid[:frame]
    #         dfidframe = dfid[dfid[:frame].==frameid,:]
    #         retval = dfidframe[:,:Row][1]
    #     end
    # end
...
const NGSIM_TRAJDATA_PATHS = [
                        Pkg.dir("NGSIM", "data", "lankershim_trajectories.txt"),
